...
function CountryList({ countries, filterData }){
  ...
  const activeRegions = validateRegionsQuery(getRegionsQuery(router.query), filterData.regionIndexes)
  ...
  const activeNumbers = getNumbersQueryData(activeHidden, router.query, filterData)
  const countriesFilteredByNumbers = filterCountriesByNumbers(countriesFilteredByRegion, activeNumbers.activeNumberFilters, activeNumbers.currentSelections);
