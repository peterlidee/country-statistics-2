...
describe('function filterCountriesByRegion(countries, regionIndexes, activeRegions)', () => {
  ...
  describe('It ignores non existing regions or subregions', () => {

    test('single non existing', () => {
      const filteredCountries = filterCountriesByRegion(extraDataCountries, regionIndexes, ['blabla'])
      expect(filteredCountries).toHaveLength(0)
    })
  
    test('single non existing, single existing', () => {
      const filteredCountries = filterCountriesByRegion(extraDataCountries, regionIndexes, ['blabla', 'Western Europe'])
      expect(filteredCountries).toHaveLength(1)
      expect(filteredCountries[0].cca3).toBe('BEL')
    })

    test('multiple non existing, multiple existing', () => {
      const filteredCountries = filterCountriesByRegion(extraDataCountries, regionIndexes, ['foo', 'Western Europe', 'Northern Europe', 'bar'])
      expect(filteredCountries).toHaveLength(2)
      expect(filteredCountries[0].cca3).toBe('BEL')
      expect(filteredCountries[1].cca3).toBe('DNK')
    })
