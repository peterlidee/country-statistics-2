export default function extractPopulationChartData(records){
  // records is an array with 2 type of items
  // female and male population totals per year
  // the year should span from 2001-2020
  ...
  const femaleTotal = [], maleTotal = [], combinedTotal = [], years = [];
  ...
  for(let i = 0; i < records.length; i++){
    if(records[i].indicator.id == "SP.POP.TOTL.MA.IN"){ // male
      maleItems.push({
