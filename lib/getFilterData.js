...
// this function returns an array with n falses
// f.e. n = 3 => [false, false, false]
function makeArrayWithFalses(length){
  const newArray = [];
...
function getRegionData(countries){ 
  ...
  Object.keys(regionNames).sort((a, b) => a > b).map(regionName => {
    fauxRegionState[regionName] = {
      regionActive: false,
      subregionNames: [...regionNames[regionName]].sort((a, b) => a > b),
      subregionActive: makeArrayWithFalses(regionNames[regionName].size),
    }
  });

  return { fauxRegionState, regionIndexes }
}
