...
function calculateNumberSliderSetting(min, max, aroundHowManySteps = 15){
...
// returns x
// min + ( x . num ) >= max
function stepsNeeded(num, min, max){
  let counter = 0;
...
function selectOptimalSteps(arr, aroundHowManySteps){
  let current = false;
  let index = 0;
  arr.map((item, i) => {
