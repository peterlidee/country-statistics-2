...
export function calculateNumberSliderSetting(min, max, aroundHowManySteps = 15){
  ...
  const numberOfSteps = stepOptions.map(stepOption => stepsNeeded(stepOption, min, max));

  // now we have an array of steps, now we need to select the optimal one
  // this would be the one closest to aroundHowManySteps
  const selectedStepIndex = selectOptimalSteps(numberOfSteps, aroundHowManySteps);
  ...
  let sliderEnd = (max % selectedStep === 0) ? max: max - (max % selectedStep) + selectedStep;
...
export function closestRoundish(num){
  ...
  return [minClosest, (maxClosest/4) , (maxClosest/2), (maxClosest/4) + (maxClosest/2) ,maxClosest];
}

// number of steps needed to get from min to max
// returns x
// min + ( x . num ) >= max
export function stepsNeeded(num, min, max){
  let counter = 0;
...
// select the optimal number of steps, closest to aroundHowManySteps
// takes an array of number of steps
// return the index of the optimal one
export function selectOptimalSteps(arr, aroundHowManySteps){
  let current = false;
  let index = 0;
  arr.map((item, i) => {
    const diff = Math.abs(aroundHowManySteps - item);
    if(!current){
      current =  diff;
      index = i;
